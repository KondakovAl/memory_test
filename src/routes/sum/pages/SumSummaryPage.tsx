...
export function SumSummaryPage() {
  const navigate = useNavigate();
  const { roundResults, settings } = useSumSession();
      ...
      <Paper p="xl" radius="md" withBorder w="100%">
        ...
        <Stack gap="xs" mb="xl">
          ...
          <Text size="sm" c="dimmed">
            Ваше время было на <strong>{Math.round(percentFaster)}%</strong>{" "}
            быстрее максимального ({maxTimeSec} с).
          </Text>
        </Stack>
        <Title order={4} mb="md">
        ...
        <Button
          mt="md"
          variant="light"
          onClick={() => navigate({ to: "/sum/settings" })}
        >
          К настройкам
        </Button>
      </Paper>
